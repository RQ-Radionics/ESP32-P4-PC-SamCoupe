...
// ESP32-P4 (RISC-V) has hardware FPU for 'float' but NOT for 'double'.
// All double arithmetic is software-emulated (~10x slower than float).
// Use float throughout — precision is more than sufficient for audio output.
void scale_for_output(unsigned int left_input, unsigned int right_input,
	float oversample_scalar, bool highpass, float boost,
	float& filterout_z1_left, float& filterout_z1_right,
	BYTE* &pBuffer)
{
	float float_left  = (float)left_input  / oversample_scalar * DEFAULT_UNBOOSTED_MULTIPLIER;
	float float_right = (float)right_input / oversample_scalar * DEFAULT_UNBOOSTED_MULTIPLIER;

	if (highpass)
	{
		// DC-blocking highpass: cutoff ~5 Hz at 22050 Hz sample rate
		const float b1 = 0.99928787f;
	...
	signed short left_output  = (signed short)(float_left  > 32767.f ? 32767 : float_left  < -32768.f ? -32768 : float_left);
	signed short right_output = (signed short)(float_right > 32767.f ? 32767 : float_right < -32768.f ? -32768 : float_right);
...
void CSAASoundInternal::GenerateMany(BYTE* pBuffer, unsigned long nSamples)
{
	...
	static float filterout_z1_left_mixed = 0, filterout_z1_right_mixed = 0;
	...
	float nBoost = (float)m_Config.m_nBoost;
#else
	float nBoost = (float)DEFAULT_BOOST;
#endif
#else
	float nBoost = 1.0f;
#endif

	float oversample = float(1 << m_nOversample);
...
void CSAASoundInternal::GenerateManySeparate(BYTE** pBuffers, unsigned long nSamples)
{
	...
	static float filterout_z1_left_0 = 0, filterout_z1_right_0 = 0;
	static float filterout_z1_left_1 = 0, filterout_z1_right_1 = 0;
	static float filterout_z1_left_2 = 0, filterout_z1_right_2 = 0;
	static float filterout_z1_left_3 = 0, filterout_z1_right_3 = 0;
	static float filterout_z1_left_4 = 0, filterout_z1_right_4 = 0;
	static float filterout_z1_left_5 = 0, filterout_z1_right_5 = 0;
