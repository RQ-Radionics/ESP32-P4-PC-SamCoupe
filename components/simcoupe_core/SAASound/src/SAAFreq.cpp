...
#else
// Pointer — allocated in PSRAM on first _SetClockRate() call
unsigned long* CSAAFreq::m_FreqTable = nullptr;
...
void CSAAFreq::_SetClockRate(int nClockRate)
{
	...
	if (nClockRate != m_nClockRate)
	{
		...
		if (!m_FreqTable)
		{
			// Allocate in internal DRAM (not PSRAM) — accessed every Tick(),
			// PSRAM latency would add ~100ns per lookup vs ~5ns for DRAM.
			m_FreqTable = static_cast<unsigned long*>(
				heap_caps_malloc(2048 * sizeof(unsigned long), MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT));
			if (!m_FreqTable)
				esp_system_abort("CSAAFreq: failed to allocate m_FreqTable in DRAM");
		}
		int ix = 0;
